...
class Entropy:
    width: int
    depth: int
    alpha: float = 2.0  # Rényi order; alpha→1 approaches Shannon entropy
    ...
    def calculate_entropy(self, cms: CountMinSketch, items: List[str]) -> Tuple[float, float, int]:
        ...
        if abs(alpha - 1.0) < 1e-9:
            ...
            # Rényi entropy of order alpha: H_alpha = 1/(1-alpha) * log2(sum(p_i^alpha))
            sum_p_alpha = 0.0
            for item in items:
                freq = cms.estimate(item)
                if freq > 0:
                    non_zero_items += 1
                    prob = freq / cms.total_count
                    sum_p_alpha += math.pow(prob, alpha)
            if sum_p_alpha > 0:
                entropy = math.log2(sum_p_alpha) / (1.0 - alpha)
            else:
                entropy = 0.0
...
def calculate_all_entropies(flows: List[Flow], width: int = 1024, depth: int = 5, alpha: float = 2.0) -> EntropyMetrics:
    entropy_calc = Entropy(width=width, depth=depth, alpha=alpha)
...
def calculate_baseline_entropies(baseline_metrics: List[EntropyMetrics]) -> Dict[str, float]:
    ...
    baseline = {
        "total_flows": statistics.mean([m.total_flows for m in baseline_metrics]),
        "unique_src_ips": statistics.mean([m.unique_src_ips for m in baseline_metrics]),
        "unique_dst_ips": statistics.mean([m.unique_dst_ips for m in baseline_metrics]),
        "unique_src_ports": statistics.mean([m.unique_src_ports for m in baseline_metrics]),
        "unique_dst_ports": statistics.mean([m.unique_dst_ports for m in baseline_metrics]),
        "src_ip_entropy": statistics.mean([m.src_ip_entropy for m in baseline_metrics]),
        "src_ip_cardinality": statistics.mean([m.src_ip_cardinality for m in baseline_metrics]),
        "dst_ip_entropy": statistics.mean([m.dst_ip_entropy for m in baseline_metrics]),
        "dst_ip_cardinality": statistics.mean([m.dst_ip_cardinality for m in baseline_metrics]),
        "src_port_entropy": statistics.mean([m.src_port_entropy for m in baseline_metrics]),
        "src_port_cardinality": statistics.mean([m.src_port_cardinality for m in baseline_metrics]),
        "dst_port_entropy": statistics.mean([m.dst_port_entropy for m in baseline_metrics]),
        "dst_port_cardinality": statistics.mean([m.dst_port_cardinality for m in baseline_metrics]),
        "src_ip_uniformity": statistics.mean([m.src_ip_uniformity for m in baseline_metrics]),
        "dst_ip_uniformity": statistics.mean([m.dst_ip_uniformity for m in baseline_metrics]),
        "src_dst_ratio": statistics.mean([m.src_dst_ratio for m in baseline_metrics]),
    }
    return baseline
